...
namespace LatexConverter
{
    ...
    public class ScreenReaderVisitor : BaseVisitor<string>
    {
        ...
        {
            if (System.Text.RegularExpressions.Regex.IsMatch(node.Text, @"[a-zA-Z0-9]+(-[a-zA-Z0-9]+)+")) return node.Text;
        ...
        {
            if (Dictionaries.ScreenReaderTemplateMap.TryGetValue(node.Command, out var template))
            {
