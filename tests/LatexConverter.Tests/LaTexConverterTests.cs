...
namespace LatexConverter.Tests
{
    public class LaTexConverterTests
    {
        ...
        [InlineData(@"\iint_{S} \vec{E} \cdot d\vec{A}", "iint_(S) E * dA")]
        [InlineData(@"\oiint_{S} \vec{B} \cdot d\vec{A} = 0", "oiint_(S) B * dA = 0")]
        [InlineData(@"\oint_{C} \vec{B} \cdot d\vec{l} = \mu_0 I", "oint_(C) B * dl = mu_0 I")]
        [InlineData(@"E = mc^2 \pm \delta E", "E = mc^2 pm delta E")]
        [InlineData(@"F = \mp kx", "F = mp kx")]
        [InlineData(@"\vec{a} \cdot \vec{b}", "a * b")]
        [InlineData(@"\vec{a} \times \vec{b}", "a times b")]
        [InlineData(@"\nabla \cdot \vec{E} = \frac{\rho}{\epsilon_0}", "nabla * E = rho/epsilon_0")]
        [InlineData(@"\sin^{-1}(x)", "arcsin(x)")]
        [InlineData(@"\cos^{-1}(y)", "arccos(y)")]
        [InlineData(@"\tan^{-1}(z)", "arctan(z)")]
        [InlineData(@"\sinh(x)", "sinh(x)")]
        [InlineData(@"\cosh(y)", "cosh(y)")]
        [InlineData(@"\tanh(z)", "tanh(z)")]
        [InlineData(@"x \quad y", "x y")]
        [InlineData(@"x \qquad y", "x  y")]
        [InlineData(@"1, 2, \cdots, n", "1, 2, ..., n")]
        [InlineData(@"A_{i,j} = \ddots", "A_(i,j) = ...")]
        [InlineData(@"\vdots", "...")]
        ...
        [InlineData(@"\iint_{S} \vec{E} \cdot d\vec{A}", "∬ₛ E⃗ ⋅ dA⃗")]
        [InlineData(@"\oiint_{S} \vec{B} \cdot d\vec{A} = 0", "∯ₛ B⃗ ⋅ dA⃗ = 0")]
        [InlineData(@"\oint_{C} \vec{B} \cdot d\vec{l} = \mu_0 I", "∮꜀ B⃗ ⋅ dl⃗ = μ₀ I")]
        [InlineData(@"E = mc^2 \pm \delta E", "E = mc² ± δ E")]
        [InlineData(@"F = \mp kx", "F = ∓ kx")]
        [InlineData(@"\vec{a} \cdot \vec{b}", "a⃗ ⋅ b⃗")]
        [InlineData(@"\vec{a} \times \vec{b}", "a⃗ × b⃗")]
        [InlineData(@"\nabla \cdot \vec{E} = \frac{\rho}{\epsilon_0}", "∇ ⋅ E⃗ = ρ / ε₀")]
        [InlineData(@"\sin^{-1}(x)", "arcsin(x)")]
        [InlineData(@"\cos^{-1}(y)", "arccos(y)")]
        [InlineData(@"\tan^{-1}(z)", "arctan(z)")]
        [InlineData(@"\sinh(x)", "sinh(x)")]
        ...
        [InlineData(@"\iint_{S} \vec{E} \cdot d\vec{A}", "double integral over S of vector E dot d vector A")]
        [InlineData(@"\oiint_{S} \vec{B} \cdot d\vec{A} = 0", "closed surface integral over S of vector B dot d vector A equals 0")]
        [InlineData(@"\oint_{C} \vec{B} \cdot d\vec{l} = \mu_0 I", "closed line integral over C of vector B dot d vector l equals mu subscript 0 I")]
        [InlineData(@"E = mc^2 \pm \delta E", "E equals mc squared plus-minus delta E")]
        [InlineData(@"F = \mp kx", "F equals minus-plus kx")]
        [InlineData(@"\vec{a} \cdot \vec{b}", "vector a dot vector b")]
        [InlineData(@"\vec{a} \times \vec{b}", "vector a cross vector b")]
        [InlineData(@"\nabla \cdot \vec{E} = \frac{\rho}{\epsilon_0}", "nabla dot vector E equals fraction with numerator rho and denominator epsilon subscript 0")]
        [InlineData(@"\sin^{-1}(x)", "arcsin of x")]
        [InlineData(@"\cos^{-1}(y)", "arccos of y")]
        [InlineData(@"\tan^{-1}(z)", "arctan of z")]
        [InlineData(@"\sinh(x)", "hyperbolic sine of x")]
        [InlineData(@"\cosh(y)", "hyperbolic cosine of y")]
        [InlineData(@"\tanh(z)", "hyperbolic tangent of z")]
        [InlineData(@"x \quad y", "x y")]
        [InlineData(@"x \qquad y", "x  y")]
        [InlineData(@"1, 2, \cdots, n", "1, 2, ..., n")]
        [InlineData(@"A_{i,j} = \ddots", "A subscript i,j equals diagonal dots")]
        [InlineData(@"\vdots", "vertical dots")]
        [InlineData(@"\emptyset", "empty set")]
